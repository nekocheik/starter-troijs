...
import type { ComponentPropsOptions, PropType, InjectionKey, ComponentPublicInstance } from 'vue';
import {
  Color,
  Material,
  MeshBasicMaterial,
  ...
  Texture,
  ShadowMaterial as TShadowMaterial,
} from 'three';
import { MeshInjectionKey } from '../meshes/Mesh';
import type { MeshInterface } from '../meshes/Mesh';
import { bindObjectProp, propsValues } from '../tools';
import type {
  BasicMaterialPropsInterface,
  LambertMaterialPropsInterface,
  MaterialPropsInterface,
  PhongMaterialPropsInterface,
  PhysicalMaterialPropsInterface,
  PointsMaterialPropsInterface,
  StandardMaterialPropsInterface,
  ToonMaterialPropsInterface,
} from './types';
...
export function materialComponent<P extends Readonly<ComponentPropsOptions>>(name: string, props: P, createMaterial: { (opts: any): Material }) {
...
export const ShadowMaterial = materialComponent(
  'ShadowMaterial',
  { color: { type: String, default: '#000000' }, props: { type: Object as PropType<MaterialPropsInterface>, default: () => ({}) } },
  (opts) => new TShadowMaterial(opts),
);
